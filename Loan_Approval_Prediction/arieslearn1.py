...
def classify_data(observation, tree):
    """
    Classifies an observation using a decision tree.
    
    Args:
        observation (dict): Data point to classify
    ...
    # Handle numerical split (<= threshold)
    if question.split()[1] == '<=':
        feature = question.split()[0]
        threshold = float(question.split()[2])
    ...
    # Handle categorical split (in category set)
    else:
        feature = question.split()[0]
        categories = question.split()[2]
        
        if observation[feature] in categories:
            answer = tree[question][0]
        else:
            answer = tree[question][1]
    
    # Base case: return answer if leaf node
    if not isinstance(answer, dict):
        return answer
    
    # Recursive case: traverse subtree
